...
from sentientresearchagent.core.system_manager import (
    SystemManagerV2 as ConcreteSystemManager,
)
...
class ResearchRequest(BaseModel):
    ...
    max_steps: int = Field(
        default=25, ge=5, le=100, description="Maximum execution steps"
    )
    max_concurrent_nodes: Optional[int] = Field(
        default=None, ge=1, le=20, description="Override concurrent nodes"
    )
    skip_atomization: Optional[bool] = Field(
        default=None, description="Override atomization setting"
    )
    save_state: bool = Field(
        default=False, description="Whether to save execution state"
    )
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print(
            f"✅ System initialized - Skip atomization: {config.execution.skip_atomization}"
        )
    ...
    if profile not in agent_cache:
        try:
            agent = LightweightSentientAgent.create_with_profile(
                profile_name=profile, system_manager=system_manager
            )
...
async def research_async_endpoint(
