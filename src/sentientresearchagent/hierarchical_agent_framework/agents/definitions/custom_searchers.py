...
try:
    ...
    logger.warning(
        "Warning: openai module not found. OpenAICustomSearchAdapter will not be usable."
    )
    OpenAI = None
    AsyncOpenAI = None
...
try:
    ...
    logger.warning(
        "Warning: google-genai module not found. GeminiCustomSearchAdapter will not be usable."
    )
    genai = None
...
try:
    ...
    logger.warning(
        "Warning: wikipediaapi module not found. Wikipedia enhancement will not be available."
    )
    wikipediaapi = None
...
try:
    ...
    logger.warning(
        "Warning: exa_py module not found. ExaCustomSearchAdapter will not be usable."
    )
    Exa = None
...
try:
    ...
    logger.warning(
        "Warning: litellm module not found. ExaCustomSearchAdapter will not be usable."
    )
    acompletion = None
...
from sentientresearchagent.hierarchical_agent_framework.agents.base_adapter import (
    BaseAdapter,
)
...
if TYPE_CHECKING:
    ...
    from sentientresearchagent.hierarchical_agent_framework.tracing.manager import (
        TraceManager,
    )
...
class OpenAICustomSearchAdapter(BaseAdapter):
    ...
    def __init__(
        ...
        search_context_size: str = "standard",
        **kwargs,
    ):
        super().__init__(self.adapter_name)
        if AsyncOpenAI is None:
            raise ImportError(
                "AsyncOpenAI client from openai library is not available. Please install or update 'openai'."
            )

        self.use_openrouter = use_openrouter
        self.search_context_size = (
            search_context_size  # Can be "medium", "high", or "low"
        )
        ...
        if use_openrouter:
            ...
            if not api_key:
                raise ValueError(
                    "OPENAI_API_KEY environment variable is required for OpenAICustomSearchAdapter"
                )
        ...
        logger.info(
            f"Initialized {self.agent_name} with model: {self.model_id} (OpenRouter: {use_openrouter}, search_context_size: {search_context_size})"
        )
    ...
    async def process(
        ...
        logger.info(
            f"  Adapter '{self.adapter_name}': Processing node {node.task_id} (Query: '{query[:100]}...') with OpenAI model {self.model_id}"
        )
        ...
        trace_manager.update_stage(
            ...
            input_context={
                ...
                "has_dependency_context": any(
                    item.content_type_description == "explicit_dependency_output"
                    for item in agent_task_input.relevant_context_items
                ),
        ...
        output_text_with_citations = (
            f"Error: Could not retrieve output_text for query: {query}"  # Default error
        )
        ...
        try:
            ...
            if not isinstance(self.client, AsyncOpenAI) or not hasattr(
                self.client.responses, "create"
            ):
                logger.error(
                    f"  {self.adapter_name}: Invalid OpenAI client. Expected AsyncOpenAI with responses.create. Got: {type(self.client)}"
                )
...
RETRIEVED DATA:"""
            ...
            logger.debug(
                f"Complete LLM input for {self.adapter_name}:\n{enhanced_query}"
            )
            ...
            if self.use_openrouter:
                ...
                if hasattr(api_response, "choices") and api_response.choices:
                    ...
                    if len(api_response.choices) > 0 and hasattr(
                        api_response.choices[0], "message"
                    ):
                        if api_response.choices[0].message and hasattr(
                            api_response.choices[0].message, "content"
                        ):
                            raw_output = api_response.choices[0].message.content
                        else:
                            logger.error(
                                f"    {self.adapter_name}: No message content in OpenRouter response"
                            )
                            raw_output = None
                    else:
                        logger.error(
                            f"    {self.adapter_name}: Empty choices array or no message in OpenRouter response"
                        )
                        raw_output = None
                else:
                    logger.error(
                        f"    {self.adapter_name}: No choices found in OpenRouter response"
                    )
                    raw_output = None
            else:
                ...
                if hasattr(api_response, "output_text") and api_response.output_text:
                    ...
                    logger.error(
                        f"    {self.adapter_name}: 'output_text' not found or empty in API response. Main output will be error message."
                    )
                    raw_output = None
            ...
            if raw_output:
                ...
                logger.success(
                    f"    {self.adapter_name}: Retrieved 'output_text' (length: {len(output_text_with_citations)})."
                )
            else:
            ...
            try:
                if (
                    not self.use_openrouter
                    and hasattr(api_response, "output")
                    and isinstance(api_response.output, list)
                    and len(api_response.output) > 1
                    and hasattr(api_response.output[1], "content")
                    and isinstance(api_response.output[1].content, list)
                    and len(api_response.output[1].content) > 0
                ):
                    ...
                    if hasattr(content_item, "text") and content_item.text:
                        ...
                        logger.success(
                            f"    {self.adapter_name}: Retrieved nested 'text_content' (length: {len(parsed_text_content)})."
                        )
                    else:
                        logger.warning(
                            f"    {self.adapter_name}: Nested 'text' attribute not found or empty."
                        )

                    if hasattr(content_item, "annotations") and isinstance(
                        ...
                        for ann_obj in content_item.annotations:
                            ...
                            if (
                                ann_dict["url"]
                                and ann_dict["start_index"] != -1
                                and ann_dict["end_index"] != -1
                            ):
                                temp_raw_annotations.append(ann_dict)
                            else:
                                logger.warning(
                                    f"    {self.adapter_name}: Skipping invalid nested annotation object: {ann_obj}"
                                )

                        if temp_raw_annotations:
                            raw_annotations_data = temp_raw_annotations  # Assign if we got valid annotations
                            logger.success(
                                f"    {self.adapter_name}: Retrieved {len(raw_annotations_data)} nested 'annotations'."
                            )
                    else:
                        logger.warning(
                            f"    {self.adapter_name}: Nested 'annotations' attribute not found or not a list."
                        )
                else:
                    logger.warning(
                        f"    {self.adapter_name}: Nested API response structure 'output[1].content[0]' not found. No supplementary text/annotations."
                    )
            except (AttributeError, IndexError, TypeError) as e:
                logger.warning(
                    f"    {self.adapter_name}: Could not parse nested content from API response: {e}"
                )
            ...
            for ann_data in raw_annotations_data:
                try:
                    ...
                    logger.warning(
                        f"    {self.adapter_name}: Error parsing an annotation dict: {ann_data}, Error: {e_pydantic}"
                    )
            ...
            clean_output = {
                ...
                "citations": [
                    ann.model_dump() for ann in parsed_annotations
                ],  # Include citations for transparency
            }

            main_output_preview = (
                clean_output["output_text"][:150] + "..."
                if len(clean_output["output_text"]) > 150
                else clean_output["output_text"]
            )
            logger.success(
                f"  Adapter '{self.adapter_name}': Processed. Main output: '{main_output_preview}', Annotations: {len(parsed_annotations)}"
            )
            ...
            return {"query_used": query, "output_text": f"API Call Failed: {e}"}
...
class GeminiCustomSearchAdapter(BaseAdapter):
    ...
    def __init__(self, gemini_client=None, model_id: str = "gemini-2.5-flash"):
        ...
        if genai is None:
            raise ImportError(
                "google-genai module is not available. Please install 'google-genai'."
            )

        # Debug: Let's see what environment variables are actually set
        api_key = os.getenv("GOOGLE_API_KEY") or os.getenv("GEMINI_API_KEY")
        logger.info(
            f"🔍 DEBUG: GOOGLE_API_KEY/GEMINI_API_KEY from os.getenv: {api_key[:10] if api_key else 'None'}...{api_key[-4:] if api_key and len(api_key) > 10 else ''}"
        )
        ...
        for key, value in os.environ.items():
            if "GOOGLE" in key.upper() or "GEMINI" in key.upper():
                logger.info(
                    f"🔍 DEBUG: Found env var {key}: {value[:10] if value else 'None'}...{value[-4:] if value and len(value) > 10 else ''}"
                )

        if not api_key:
            raise ValueError(
                "GOOGLE_API_KEY or GEMINI_API_KEY environment variable is required for GeminiCustomSearchAdapter"
            )
        ...
        self.model_id = model_id
        logger.info(
            f"Initialized {self.agent_name} with model: {self.model_id} (API key: {api_key[:10]}...{api_key[-4:]})"
        )
    ...
    async def process(
        ...
        query = agent_task_input.current_goal
        logger.info(
            f"  Adapter '{self.adapter_name}': Processing node {node.task_id} (Query: '{query[:100]}...') with Gemini model {self.model_id}"
        )
...
RETRIEVED DATA:"""
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if "event loop" in str(e).lower():
                        ...
                        logger.warning(
                            f"Event loop issue detected, recreating client: {e}"
                        )
                    ...
                    if any(
                        ...
                        if attempt < max_retries - 1:
                            ...
                            logger.warning(
                                f"Rate limit hit (attempt {attempt + 1}/{max_retries}), retrying in {delay:.1f}s: {e}"
                            )
                    ...
                    # Handle other API errors with shorter retry
                        ...
                        if attempt < max_retries - 1:
                            delay = base_delay * (2**attempt)
                            logger.warning(
                                f"API error (attempt {attempt + 1}/{max_retries}), retrying in {delay:.1f}s: {e}"
                            )
                            await asyncio.sleep(delay)
                            continue
                        else:
            ...
            if hasattr(api_response, "text") and api_response.text:
                ...
                parsed_text_content = (
                    output_text_with_citations  # Same content for both fields
                )
                logger.success(
                    f"    {self.adapter_name}: Retrieved response text (length: {len(output_text_with_citations)})."
                )
            else:
                logger.error(
                    f"    {self.adapter_name}: 'text' not found or empty in API response. Main output will be error message."
                )
                logger.debug(
                    f"    {self.adapter_name}: API response attributes: {dir(api_response) if api_response else 'None'}"
                )
                # Keep the default error message for output_text_with_citations
            ...
            if (
                hasattr(api_response, "candidates")
                and api_response.candidates is not None
                and isinstance(api_response.candidates, (list, tuple))
                and len(api_response.candidates) > 0
                and hasattr(api_response.candidates[0], "grounding_metadata")
                and api_response.candidates[0].grounding_metadata is not None
                and hasattr(
                    api_response.candidates[0].grounding_metadata, "grounding_chunks"
                )
                and api_response.candidates[0].grounding_metadata.grounding_chunks
                is not None
            ):
                grounding_chunks = api_response.candidates[
                    0
                    ...
                    for i, chunk in enumerate(grounding_chunks):
                        if hasattr(chunk, "web") and chunk.web:
                            ...
                            ann_dict = {
                                ...
                                "end_index": len(output_text_with_citations)
                                if output_text_with_citations
                                else 0,
                                "type": "url_citation",
                            }
                            ...
                            if ann_dict["url"]:
                                ...
                                logger.warning(
                                    f"    {self.adapter_name}: Skipping grounding chunk without URL: {chunk}"
                                )
                else:
                    logger.warning(
                        f"    {self.adapter_name}: grounding_chunks is not iterable: {type(grounding_chunks)}"
                    )

                if raw_annotations_data:
                    logger.success(
                        f"    {self.adapter_name}: Retrieved {len(raw_annotations_data)} grounding citations."
                    )
            else:
                logger.warning(
                    f"    {self.adapter_name}: No grounding metadata found in API response."
                )
            ...
            if parsed_annotations:
                for ann in parsed_annotations:
                    if ann.url and "wikipedia.org" in ann.url.lower():
                        logger.info(
                            f"    {self.adapter_name}: Found Wikipedia citation: {ann.url}"
                        )
                        wiki_content = await fetch_wikipedia_content(ann.url)
                        if wiki_content:
                            wikipedia_contents.append(wiki_content)
                            logger.success(
                                f"    {self.adapter_name}: Successfully fetched Wikipedia content for: {ann.title or ann.url}"
                            )
            ...
            if wikipedia_contents:
                ...
                logger.info(
                    f"    {self.adapter_name}: Enhanced output with {len(wikipedia_contents)} Wikipedia article(s)"
                )
            ...
            logger.success(
                f"  Adapter '{self.adapter_name}': Processed. Main output: '{main_output_preview}', Citations processed: {len(parsed_annotations)}"
            )
            node.output_type_description = "custom_searcher_output"
