...
__all__ = [
    "ModelConfig",
    "ToolConfig",
    "BaseDataToolkitParams",
    "BinanceToolkitParams",
    "CoingeckoToolkitParams",
    "ArkhamToolkitParams",
    "DefiLlamaToolkitParams",
    "E2BToolsParams",
    "ToolkitConfig",
    "ActionKey",
    "RegistrationConfig",
...
ModelProviderType = Literal[
    "litellm", "openai", "fireworks", "fireworks_ai", "google", "gemini"
]

# Agent types
AgentType = Literal[
    "planner", "executor", "aggregator", "atomizer", "plan_modifier", "custom_search"
]
...
class ModelConfig(BaseModel):
    ...
    temperature: Optional[float] = Field(
        None, ge=0.0, le=2.0, description="Model temperature for response randomness"
    )
    max_tokens: Optional[int] = Field(
        None, gt=0, description="Maximum tokens for model response"
    )
    ...
    def validate_environment(self):
        ...
        if provider == "litellm":
            ...
            elif model_id.startswith("fireworks_ai/") or model_id.startswith(
                "fireworks/"
            ):
            ...
            if not os.getenv("FIREWORKS_AI_API_KEY"):
                raise ValueError(
                    "Fireworks AI provider requires FIREWORKS_AI_API_KEY environment variable"
                )
...
class ToolConfig(BaseModel):
    ...
    def validate_tool_params(self):
        ...
        if tool_name == "PythonTools" and params is not None:
            ...
            if "save_and_run" in params and not isinstance(
                params["save_and_run"], bool
            ):
...
class BaseDataToolkitParams(BaseModel):
    ...
    data_dir: Union[str, Path] = Field(
        "./data", description="Directory for storing parquet files"
    )
    parquet_threshold: int = Field(
        500, gt=0, description="Minimum size to trigger parquet storage"
    )
...
class BinanceToolkitParams(BaseDataToolkitParams):
    ...
    data_dir: Union[str, Path] = Field(
        "./data/binance", description="Directory for storing parquet files"
    )

    # Binance-specific fields
    default_market_type: MarketType = Field(
        "spot", description="Default market type for API calls"
    )
    symbols: Optional[List[str]] = Field(
        None, description="List of trading symbols to filter"
    )
    api_key: str = Field(
    ...
    def validate_required_credentials(self):
        ...
        if not self.api_key:
            raise ValueError(
                "Binance API key is required. Set BINANCE_API_KEY environment variable."
            )
        if not self.api_secret:
            raise ValueError(
                "Binance API secret is required. Set BINANCE_API_SECRET environment variable."
            )
        ...
        if not self.api_key.replace("-", "").replace("_", "").isalnum():
            raise ValueError(
                "Binance API key must contain only alphanumeric characters, hyphens, and underscores."
            )
        ...
        if not self.api_secret.replace("-", "").replace("_", "").isalnum():
            raise ValueError(
                "Binance API secret must contain only alphanumeric characters, hyphens, and underscores."
            )
...
class CoingeckoToolkitParams(BaseDataToolkitParams):
    ...
    # Override defaults from base class
    data_dir: Union[str, Path] = Field(
        "./data/coingecko", description="Directory for storing parquet files"
    )
    ...
    def validate_required_credentials(self):
        ...
        if not self.api_key:
            raise ValueError(
                "CoinGecko API key is required. Set COINGECKO_API_KEY environment variable."
            )
        ...
        if not self.api_key.startswith("CG-"):
            raise ValueError(
                "CoinGecko API key must start with 'CG-' followed by alphanumeric characters."
            )
        ...
        if not key_suffix.isalnum():
            raise ValueError(
                "CoinGecko API key suffix (after 'CG-') must contain only alphanumeric characters."
            )
...
class ArkhamToolkitParams(BaseDataToolkitParams):
    ...
    # Override defaults from base class
    data_dir: Union[str, Path] = Field(
        "./data/arkham", description="Directory for storing parquet files"
    )
    ...
    default_chain: str = Field(
        "ethereum", description="Default blockchain network for queries"
    )
    base_url: str = Field(
    ...
    supported_chains: Optional[List[str]] = Field(
        None, description="List of blockchain networks to support"
    )
    include_entity_data: bool = Field(
        True, description="Include entity attribution data when available"
    )
    cache_ttl_seconds: int = Field(
        1800, gt=0, description="Cache time-to-live for API responses"
    )
    ...
    def validate_default_chain(cls, v):
        ...
        if v.lower() not in valid_chains:
            raise ValueError(
                f"Unsupported default_chain '{v}'. Supported: {valid_chains}"
            )
        return v.lower()
    ...
    def validate_supported_chains(cls, v):
        ...
        for chain in v:
            ...
            if chain_lower not in valid_chains:
                raise ValueError(
                    f"Unsupported chain '{chain}'. Supported: {valid_chains}"
                )
...
class DefiLlamaToolkitParams(BaseDataToolkitParams):
    ...
    # Override defaults from base class
    data_dir: Union[str, Path] = Field(
        "./data/defillama", description="Directory for storing parquet files"
    )
    ...
    base_url: str = Field(
        "https://api.llama.fi", description="Base URL for DefiLlama free API"
    )
    pro_base_url: str = Field(
        "https://pro-api.llama.fi", description="Base URL for DefiLlama Pro API"
    )
...
class ToolkitConfig(BaseModel):
    ...
    params: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Toolkit initialization parameters"
    )
    available_tools: Optional[List[str]] = Field(
        None, description="Specific tools to extract from toolkit"
    )
    ...
    def validate_toolkit_config(self):
        ...
        if param_class:
            ...
            if available_tools:
                ...
                invalid_tools = [
                    tool for tool in available_tools if tool not in valid_tools
                ]
                if invalid_tools:
                    raise ValueError(
    ...
    def validate_task_type(cls, v):
        ...
        if v is not None:
            ...
            if v.upper() not in valid_types:
                raise ValueError(
                    f"Invalid task_type: {v}. Must be one of {valid_types}"
                )
        return v
...
class RegistrationConfig(BaseModel):
    ...
    action_keys: Optional[List[ActionKey]] = Field(
        default_factory=list, description="Action verb and task type mappings"
    )
    named_keys: Optional[List[str]] = Field(
        default_factory=list, description="Named keys for agent lookup"
    )
...
class AdapterParams(BaseModel):
    ...
    model_config = ConfigDict(
        extra="allow"
    )  # Allow additional adapter-specific parameters

    model_id: Optional[str] = Field(None, description="Override model ID")
    search_context_size: Optional[Literal["low", "medium", "high"]] = Field(
        None, description="Search context size for custom searchers"
    )
    use_openrouter: Optional[bool] = Field(
        None, description="Use OpenRouter instead of direct API"
    )
    num_results: Optional[int] = Field(
        None, gt=0, description="Number of search results to retrieve"
    )
...
class AgentConfig(BaseModel):
    ...
    toolkits: Optional[List[ToolkitConfig]] = Field(
        default_factory=list, description="Toolkit configurations"
    )

    # Registration configuration
    registration: Optional[RegistrationConfig] = Field(
        None, description="Agent registration configuration"
    )

    # Additional parameters
    agno_params: Optional[AgnoParams] = Field(
        None, description="Additional Agno agent parameters"
    )
    adapter_params: Optional[AdapterParams] = Field(
        None, description="Additional adapter parameters"
    )
    ...
    def validate_adapter_class(cls, v):
        ...
        if v not in valid_adapters:
            raise ValueError(
                f"Invalid adapter class: {v}. Must be one of {valid_adapters}"
            )
        return v
    ...
    def normalize_tools(cls, v):
        ...
        for tool in v:
            if isinstance(tool, str):
                ...
                normalized.append(
                    tool if tool == "web_search" else ToolConfig(name=tool)
                )
    ...
    def validate_agent_config(self):
        ...
        if agent_type == "custom_search":
            ...
            if not self.model:
                raise ValueError(
                    f"Agent type '{agent_type}' requires 'model' configuration"
                )
            if not self.prompt_source:
                raise ValueError(
                    f"Agent type '{agent_type}' requires 'prompt_source' configuration"
                )
        ...
        if self.response_model:
            ...
            if (
                required_models
                and response_model not in required_models
                and None not in required_models
            ):
                raise ValueError(
...
class AgentsYAMLConfig(BaseModel):
    ...
    metadata: Optional[Dict[str, Any]] = Field(
        None, description="Configuration metadata"
    )
...
class ProfileConfig(BaseModel):
    ...
    root_planner_adapter_name: Optional[str] = Field(
        None, description="Root planner agent name for initial decomposition"
    )
    root_aggregator_adapter_name: Optional[str] = Field(
        None, description="Root aggregator agent name for final synthesis"
    )
    ...
    planner_adapter_names: Optional[Dict[str, str]] = Field(
        None, description="Task type to planner agent mappings"
    )
    executor_adapter_names: Optional[Dict[str, str]] = Field(
        None, description="Task type to executor agent mappings"
    )
    aggregator_adapter_names: Optional[Dict[str, str]] = Field(
        None, description="Task type to aggregator agent mappings"
    )

    # Special agents
    atomizer_adapter_name: Optional[str] = Field(
        None, description="Atomizer agent name"
    )
    aggregator_adapter_name: Optional[str] = Field(
        None, description="Fallback aggregator agent name"
    )
    plan_modifier_adapter_name: Optional[str] = Field(
        None, description="Plan modifier agent name"
    )

    # Default agents
    default_planner_adapter_name: Optional[str] = Field(
        None, description="Default planner agent name"
    )
    default_executor_adapter_name: Optional[str] = Field(
        None, description="Default executor agent name"
    )
    default_node_agent_name_prefix: Optional[str] = Field(
        None, description="Prefix for node agent names"
    )

    @field_validator(
        "planner_adapter_names", "executor_adapter_names", "aggregator_adapter_names"
    )
    @classmethod
