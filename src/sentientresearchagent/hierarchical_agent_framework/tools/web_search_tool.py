...
from sentientresearchagent.hierarchical_agent_framework.agents.registry import (
    AgentRegistry,
)
...
Answer:"""
        ...
        if openai_key:
            ...
            logger.opt(colors=True).info(
                "<green>[API_KEY_MONITOR]</green> Using OpenAI API with base URL: {} and path: /chat/completions",
                base_url,
            )
            headers = {
            ...
            payload = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a search engine. Provide concise, factual answers.",
                    },
        ...
        logger.opt(colors=True).info(
            "<green>[API_KEY_MONITOR]</green> No API keys found for web search"
        )
