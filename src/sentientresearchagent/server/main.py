...
class SentientServer:
    ...
    def __init__(self, config_input: Optional[SentientConfig] = None):
        ...
        if config_input is None:
            logger.info(
                "SentientServer: No config provided, auto-loading SentientConfig..."
            )
            ...
            logger.info(
                "SentientServer: Initialized with provided SentientConfig instance."
            )
        else:
            # This case should ideally not be hit if callers adhere to type hints.
            # If it could be a dict, we need to convert it.
            logger.warning(
                f"SentientServer: Received config_input of type {type(config_input)}, not SentientConfig. Attempting to parse as SentientConfig."
            )
            if isinstance(config_input, dict):
                try:
                    ...
                    logger.error(
                        f"SentientServer: Failed to parse dict config_input into SentientConfig: {e}. Falling back to auto_load_config."
                    )
                    self.config: SentientConfig = auto_load_config()
            else:  # Not None, not SentientConfig, not dict - unexpected.
                logger.error(
                    f"SentientServer: Unexpected config_input type {type(config_input)}. Falling back to auto_load_config."
                )
                self.config: SentientConfig = auto_load_config()
        ...
        if not isinstance(self.config, SentientConfig):
            logger.critical(
                f"SentientServer: self.config is NOT a SentientConfig object after init (type: {type(self.config)}). This is a critical error. Defaulting again."
            )
    ...
    def create(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize SystemManager with profile '{default_profile_to_use}': {e}"
            )
        ...
        self.system_manager.setup_websocket_hitl(
            self.socketio
        )  # Assumes system_manager.config is now profile-aware

        # Verify WebSocket HITL is ready
        # Accessing system_manager.config, which should be the same instance as self.config
        if (
            self.system_manager.config
            and self.system_manager.config.execution.enable_hitl
        ):
        ...
        self.project_service = ProjectService(
            self.system_manager, self.broadcast_manager.broadcast_graph_update
        )
        ...
        self.execution_service = ExecutionService(
            self.project_service, self.system_manager
        )
        ...
        logger.info(
            f"🤖 Active Agent Profile in SystemManager: {current_profile_active}"
        )
        # Also log from the main config for verification
        logger.info(
            f"🤖 Active Agent Profile in SentientConfig: {self.config.active_profile_name}"
        )
